...
async function register(req, env) {
  ...
  if (existing) return json({ error: "Email is already registered" }, 409);
...
async function paypalOrder(req, env) {
  ...
  if (!amount) return json({ error: "Invalid credit package" }, 400);

  const pkg = await env.DB.prepare(
    "SELECT id FROM credit_packages WHERE credits=? AND price=? AND active=1 LIMIT 1"
  ).bind(credits, amount).first();
  if (!pkg) return json({ error: "Credit package not found" }, 404);

  const access = await paypalToken(env);
  if (!access) return json({ error: "PayPal Sandbox is not configured" }, 503);
  ...
  const approve = data.links?.find(x => x.rel === "approve")?.href;
  if (!approve || !data.id) {
    return json({ error: "PayPal approval link was not returned" }, 502);
  }

  await env.DB.prepare(
    "INSERT INTO credit_purchases(id,user_id,package_id,credits,amount,status,paypal_order_id,created_at) VALUES(?,?,?,?,?,?,?,?)"
  ).bind(
    token(), user.id, pkg.id, credits, amount, "pending", data.id, Date.now()
  ).run();

  return json({ url: approve, order_id: data.id });
}
...
async function paypalCapture(req, env) {
  ...
  if (!purchase) return redirect("/plans/?payment=not_found");
  if (purchase.status === "completed") return redirect("/dashboard/?payment=success");
  ...
  if (!response.ok) return redirect("/plans/?payment=not_confirmed");
  ...
  await env.DB.prepare(
    ...
    "purchased",
    purchase.credits,
    planAvailable + purchasedAvailable,
...
async function paypalSubscription(req, env) {
  ...
  if (!adminPlan?.paypal_plan_id) {
    return json({ error: "PayPal plan is not configured" }, 400);
  }
  ...
  return json({ url: approve, subscription_id: data.id });
}
...
export default {
  async fetch(req, env) {
    ...
    if (path.startsWith("/admin")) {
      const adminResponse = await adminRouter(req, env);
    ...
    try {
      ...
      if ((path === "/api/auth/register" || path === "/api/register") && req.method === "POST") {
      ...
      if ((path === "/api/auth/logout" || path === "/api/logout") && req.method === "POST") {
      ...
      if (path === "/api/paypal/order" && req.method === "POST") {
        return paypalOrder(req, env);
      }

      if (path === "/api/paypal/subscription" && req.method === "POST") {
        return paypalSubscription(req, env);
      }

      if (path === "/api/paypal/capture" && req.method === "GET") {
