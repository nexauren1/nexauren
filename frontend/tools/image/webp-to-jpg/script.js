(() => {
  ...
  function useFile(selected) {
    ...
    preview.onload = () => {
      info.textContent = `${file.name} · ${preview.naturalWidth} × ${preview.naturalHeight}px · ${size(file.size)}`;
    };
    panel.classList.remove('hidden');
    setStatus('WebP ready. Choose your JPG settings.');
  }
  ...
  convertBtn.addEventListener('click', async () => {
    ...
    setStatus('Converting WebP to JPG…');
    ...
    try {
      ...
      const blob = await new Promise((resolve, reject) => {
        ...
        const timer = setTimeout(() => {
          if (!done) reject(new Error('Conversion timed out.'));
        }, 20000);
      ...
      if (!blob) throw new Error('JPG conversion is not supported.');
      ...
      setStatus(`Done · JPG created (${size(blob.size)}).`);
    } catch (error) {
      console.error(error);
