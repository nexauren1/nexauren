...
function embedCode(url, name) {
  const title = String(name || "Nexauren Event Countdown").replace(/"/g, "&quot;");
...
function render() {
  ...
  events.forEach(event => {
    ...
    clicks += Number(value.link_clicks || 0);
    const row = (value.days || []).find(day => day.day === todayKey);
    today += Number(row?.views || 0);
  });
  ...
  root.innerHTML = events.map(event => {
    ...
    const value = stats[event.id] || {};
    return `<article class="event ${paused ? "is-paused" : ""}">
      <div class="event-top"><div class="badges"><span class="badge ${paused ? "paused" : "active"}">${paused ? "PAUSED" : "ACTIVE"}</span><span class="badge">${esc(event.theme || "default")}</span></div><span class="event-id">#${esc(event.event_slug)}</span></div>
      <h2>${esc(event.name)}</h2><p>${esc(event.description || "No description")}</p>
